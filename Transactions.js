...
function updateTransactionSheet(account_id){
  ...
  if( response.success === true ){
    ...
    var has_more = true;
    Logger.log('[TX-SYNC] Starting for account: ' + account_id + ', cursor: ' + (next_cursor || '(none)'));

    // Paginate through all available pages
    while(has_more){
      let transactions = getPlaidTransactionSyncData( account_id, next_cursor );

      if(!transactions || transactions.error || !transactions.added){
        Logger.log('[TX-SYNC] No transaction data returned. Response: ' + JSON.stringify(transactions ? { error: transactions.error, hasAdded: !!transactions.added } : 'null/empty'));
        break;
      }
      ...
      Logger.log('[TX-SYNC] Page processed — added: ' + TransactionAdded.length + ', modified: ' + TransactionModified.length + ', removed: ' + TransactionRemoved.length + ', has_more: ' + transactions.has_more);
      has_more = !!transactions.has_more;
    }

  } else {
    Logger.log('[TX-SYNC] getAppPlaidAccountById FAILED for account: ' + account_id + '. Response: ' + JSON.stringify(response));
  }
