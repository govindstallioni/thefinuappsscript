...
function aggregateCategoryData(catAry, formatTransGroup) {
    ...
    catAry.forEach(catRow => {
        ...
        const rawActualAmount = parseAmount(actualMap[category] || 0);

        // Calculate the actual amount applied to the joint budget based on the owner share
        const adjustedActualAmount = rawActualAmount * combinedMultiplier;
